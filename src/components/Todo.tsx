...
export default function Todo() {
    ...
    const onAddSubmit: SubmitHandler<AddInputs> = (data) => {
        addData({ ...data, image: data.image?.[0] || null })
        resetAdd()
    }

    const onEditSubmit: SubmitHandler<EditInputs> = (data) => {
        if (editId === null) return
        editData({ id: editId, ...data })
        setEditId(null)
    }

    const handleOpenEdit = (todo: any) => {
        setEditId(todo.id)
        resetEdit({ name: todo.name, description: todo.description })
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <h1 className="text-lg font-medium text-slate-600 animate-pulse">Loading data...</h1>
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50 p-4">
                <div className="bg-white border border-red-100 rounded-2xl p-6 max-w-md w-full text-center shadow-xl shadow-red-50">
                    ...
                    <h1 className="text-lg font-bold text-slate-800 mb-2">Loading error</h1>
        ...
        <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
            <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-8 items-start">
                <form 
                    ...
                    className="w-full md:w-2/5 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 md:sticky md:top-8">
                    <h2 className="text-xl font-bold text-slate-800 mb-5 tracking-tight">New task</h2>
                    <div className="flex flex-col gap-4">
                        <div>
                            <input
                                className={`w-full bg-slate-50 border ${errorsAdd.name ? 'border-red-400 focus:border-red-500 focus:ring-red-50/50' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-50/50'} p-3 rounded-xl text-slate-800 outline-none transition-all placeholder:text-slate-400 text-sm`}
                                type="text"
                                placeholder="Task name"
                                {...registerAdd("name", { required: true })}
                            />
                            {errorsAdd.name && <span className="text-xs text-red-500 mt-1 block px-1">This field is required</span>}
                        </div>

                        <div>
                            <input
                                className={`w-full bg-slate-50 border ${errorsAdd.description ? 'border-red-400 focus:border-red-500 focus:ring-red-50/50' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-50/50'} p-3 rounded-xl text-slate-800 outline-none transition-all placeholder:text-slate-400 text-sm`}
                                type="text"
                                placeholder="Description details"
                                {...registerAdd("description", { required: true })}
                            />
                            {errorsAdd.description && <span className="text-xs text-red-500 mt-1 block px-1">This field is required</span>}
                        </div>

                        <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-semibold text-slate-500 px-1">Image</label>
                            <input
            ...
            {editId !== null && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <form 
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-4">Edit task</h3>
                        <div className="flex flex-col gap-4">
                            <div>
                                <input
                                    className={`w-full bg-slate-50 border ${errorsEdit.name ? 'border-red-400 focus:border-red-500 focus:ring-red-50/50' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-50/50'} p-3 rounded-xl text-slate-800 outline-none transition-all text-sm`}
                                    type="text"
                                    {...registerEdit("name", { required: true })}
                                />
                                {errorsEdit.name && <span className="text-xs text-red-500 mt-1 block px-1">This field is required</span>}
                            </div>

                            <div>
                                <input
                                    className={`w-full bg-slate-50 border ${errorsEdit.description ? 'border-red-400 focus:border-red-500 focus:ring-red-50/50' : 'border-slate-200 focus:border-blue-500 focus:ring-blue-50/50'} p-3 rounded-xl text-slate-800 outline-none transition-all text-sm`}
                                    type="text"
                                    {...registerEdit("description", { required: true })}
                                />
                                {errorsEdit.description && <span className="text-xs text-red-500 mt-1 block px-1">This field is required</span>}
                            </div>
